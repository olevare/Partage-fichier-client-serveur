...
int main (int argc, char** argv)
{
  ...
  printf("Client : Vous pouvez soit importer ou exporter pour cela taper import ou export.\n");
  ...
  while(1)
  {
    printf("Client : import, export ou exit ??\n");
...
//
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    else if(strncmp(message, "export", 6) == 0)
    {
      //variable pour un fichier
      FILE* fd;

      //envoie export au serveur
      if(send(brclient, message, sizeof(message), 0) == -1)
      {
        perror("Client : erreur du send");
        close(brclient);
      ...
      printf("Client : qu'elle fichier voulez-vous envoyer sur le serveur ?\n");

      //envoie du nom du fichier a envoyer au serveur
      fgets(message, sizeof(message), stdin);
      ...
      else
      {
        ...
        lstat(strtok(message, "\n"), &status);
        size = status.st_size; //off_t est un long pour la taille de fichier en gros

        //envoie de la taille du fichier
        if(send(brclient, &size, sizeof(off_t), 0) == -1)
        {
          perror("Client : erreur envoie taille fichier.\n");
          close(brclient);
          return EXIT_FAILURE;
        }

        printf("j'ai envoyé la taille %d\n", size);

        //envoie le nom du fichier a exporter
        if(send(brclient, message, sizeof(message), 0) == -1)
        {
          perror("Client : erreur du send");
          close(brclient);
        ...
        {
        	int a;
          if(a=send(brclient, message, sizeof(message), 0) < 0)
          {
            perror("Client : erreur envoie du fichier.\n");
            close(brclient);
            return EXIT_FAILURE;
          }
        ...
        printf("Client : le fichier a été envoyer sur le serveur !!\n");
      }
    }
